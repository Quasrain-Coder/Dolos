<template>
  <div class="game">
    <!-- Role badge -->
    <div v-if="gameStore.myRole" class="role-badge-top" :class="gameStore.myRole">
      {{ gameStore.roleLabel }}
    </div>
    ...
    <div class="game-main">
      <!-- ANSWERING -->
      <AnswerInput v-if="roomStore.phase === 'answering' && !gameStore.isDetective" />
      <div v-if="roomStore.phase === 'answering' && gameStore.isDetective" class="phase-waiting">
      ...
      <!-- VOTING -->
      <VotingPanel v-if="roomStore.phase === 'voting'" />
    ...
    <div
      v-if="(roomStore.phase === 'revealing' || roomStore.phase === 'round_end') && !gameStore.awaitingDetective"
      class="host-controls"
    >
      <button
      ...
      <div v-if="gameStore.readyPlayerIds.length > 0" class="ready-players">
        <span v-for="pid in gameStore.readyPlayerIds" :key="pid" class="ready-player-tag">✅ {{ getPlayerName(pid) }}</span>
      </div>
